            ...
            <table class="table table-striped" id="tabela">
                ...
                @forelse($exames as $exame)
                    <tr id={{$exame->id}}>
                        ...
                        <td width="15%">
                            <img width="80px" src="{{ asset('uploads/exames/'.$exame->image) }}">
                        </td>
                        <td width="15%">
                            {{ $exame->nome }}
                        </td>
                        ...
                        <td width="10%">
                            <span class="hint--top" aria-label="Editar Exames">
                                <a href="{{ route('exames.edit', $exame->id) }}" style="border-radius: 50%"
                                   class="btn btn-warning waves-effect">
                                    <i class="fa fa-pencil m-r-5"></i>
                                </a>
                            </span>
                            <span class="hint--top" aria-label="Deletar Exames">
                                <button type="button" onclick="goswet({{$exame->id}}, '{{$exame->texto}}')"
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">Cadastrar Exames</h4>
                </div>
                <form enctype="multipart/form-data"
                      action="{{ action('App\Http\Controllers\ExamesController@store')}}"
                      class="form"
                      method="post">
                    {{ csrf_field() }}
                        ...
                        <div class="row">
                            <div class="form-group col-md-12">
                                <input type="file" name="image" id="image" class="filestyle"
                                       data-buttonText="Carregar" data-placeholder="Resolução ideal 80 x 80"
                                       data-btnClass="btn-light" required accept="image/*">
                            </div>
                            <div class="form-group col-md-12">
                                <label for="nome">Nome:</label>
                                <input placeholder="Nome do Exame" name="nome"
                                       class="form-control" autofocus required
                                       maxlength="250" type="text">
                            </div>
                            <div class="form-group col-md-12">
                                <label for="descricao">Descrição:</label>
                                <input placeholder="Descrição do Exame" name="descricao"
                                       class="form-control" autofocus required
                                       maxlength="250" type="text">
                            </div>
                        </div>
        ...
        $.ajax({
          ...
          success: function(data){
            swal({
             ...
             function(){
               window.location = "{{ route('exames.index') }}";
             }
           );
          },
